...
const MENU_DATA = {
  ...
  submenus: {
    champagnes: {
      ...
      layout: 'bottles',
      items: ["Brut & Rosé", "Fresh & Sweet", "Vintages", "Limited Editions"], 
    },
    joy: {
      title: "MAISON DE JOI",
      layout: 'list_card',
      items: ["ABOUT US", "CRAFT & HERITAGE", "INSIDE MOËT", "OUR COMMITMENTS"],
      card: {
        subtitle: "INSIDE MOËT",
        title: "Gesture that became\na global icon",
        image: "https://raw.githubusercontent.com/marcelorm81/assets/db2c7a658ac0e7dc017babe421f0e85c999477f2/f1.jpg"
      }
    },
    experiences: {
      title: "EXPERIENCES",
      layout: 'list_card',
      items: ["TOURS", "DINING", "BARS"],
      card: {
        subtitle: "GUIDED CELLAR TOUR",
        title: "A timeless introduction\nto Moët & Chandon",
        image: "https://raw.githubusercontent.com/marcelorm81/assets/db2c7a658ac0e7dc017babe421f0e85c999477f2/experience.jpg"
      }
    },
    rituals: {
      title: "MOËT RITUALS",
      layout: 'list_card',
      items: ["HOW TO SERVE", "FOOD PAIRINGS", "COCKTAILS"],
      card: {
        subtitle: "OUR COCKTAILS",
        title: "Try the Impérial 1869:\nfresh citrus, floral lift, pure Moët.",
        image: "https://github.com/marcelorm81/assets/blob/b227c8555c849f2ffd6d7b53a27b819ddb0f0531/cocktail.avif?raw=true"
      }
    },
    find: {
      title: "FIND US",
      layout: 'list_card',
      items: ["OUR BARS", "OUR PARTNERS", "FIND BY PRODUCT"],
      card: {
        subtitle: "VISIT US",
        title: "Experience the magic\nin person",
        image: "https://raw.githubusercontent.com/marcelorm81/assets/refs/heads/main/Galerie-Imperiale_Copyright-James-Bort-for-Moet-Chandon-(1).avif"
      }
    }
  }
};

const PinnedHero: React.FC<PinnedHeroProps> = ({ theme = 'light' }) => {
  ...
  const updateColors = () => {
    ...
    // 2. SCROLL OVERRIDES
        ...
        if (colorOverrideRef.current === 'black') {
            ...
            glassBg = 'rgba(255, 255, 255, 0.4)'; // Updated to 0.4
        } else {
  ...
  useEffect(() => {
    ...
    const finalLogoTop = 'calc(45px + env(safe-area-inset-top))'; // Sticky state top: 45px
    const logoLeft = '20px'; // Sticky state left
    const initialScale = 3.1; // Initial Hero Scale
    const finalScale = 0.85;   
    
    // Set initial logo position (accounting for Safe Area)
    // Initial State: Top 15%, Left 20px, Scale 3.1
    ...
    // Final mask state: inset(18% 15% 5% 15% round 300px 300px 300px 300px)
    ...
    tl.to(menuIconRef.current, { opacity: 1, scale: 1, duration: 0.4, ease: 'back.out(1.7)' }, 0.8);
    ...
    tl.to(glassRef.current, { opacity: 1, duration: 0.5 }, 0.5);
    return () => { const otherTriggers = ScrollTrigger.getAll().filter(t => t.vars.trigger === pinRef.current); otherTriggers.forEach(t => t.kill()); };
  }, []);
      ...
      <div className="sticky top-0 z-50 w-full h-0 overflow-visible pointer-events-none">
         ...
         <div ref={menuOverlayRef} className="absolute top-0 left-0 w-full h-[100dvh] bg-[#FFFBF7] z-40 flex flex-col justify-start px-6 pointer-events-auto" 
              style={{ 
                  ...
                  visibility: 'hidden',
                  paddingTop: 'calc(115px + env(safe-area-inset-top))' // 45px(top) + ~30px(logo) + 40px(gap) = 115px
              }}>
            <div ref={menuListRef} className="flex flex-col w-full h-full pb-10">
                {activeSubmenu === null ? (
                    ...
                    <div className="flex flex-col h-full relative">
                        ...
                        <div className="absolute bottom-0 left-[-24px] w-[calc(100%+48px)] h-[80vh] flex flex-col justify-end z-10 pointer-events-auto">
                            {MENU_DATA.submenus[activeSubmenu].layout === 'bottles' ? (
                                <div ref={carouselContainerRef} className="w-full h-full overflow-x-auto overflow-y-hidden cursor-grab [&::-webkit-scrollbar]:hidden" style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}>
                                    <div ref={carouselTrackRef} className="flex items-end h-full px-6 pt-20 pb-1">
         ...
         <div className="relative w-full h-screen z-50 overflow-hidden">
             ...
             <svg ref={svgRef} xmlns="http://www.w3.org/2000/svg" width="318" height="34" viewBox="0 0 318 34" fill="none" className="pointer-events-auto overflow-visible transition-colors">
                <g id="moet">
                  ...
                  <path d="M43.6465 6.60738C37.2442 6.60738 32.0599 12.4685 32.0599 19.8537C32.0599 27.239 37.2522 33.2136 43.6465 33.2136C50.0408 33.2136 55.2412 27.2309 55.2412 19.8537C55.2412 12.4766 50.0488 6.60738 43.6465 6.60738ZM43.6706 31.4788C39.568 31.4788 36.8356 26.4769 36.8356 19.8862C36.8356 13.2954 39.568 8.41517 43.6706 8.41517C47.7731 8.41517 50.5055 13.2954 50.5055 19.8862C50.5055 26.4769 47.7731 31.4788 43.6706 31.4788Z" fill="#FFFBF7"/>
                  <path d="M80.0878 7.12591L78.9339 12.2575L82.3394 8.86074H88.4692V29.9058L85.6487 32.7026H95.6488L93.0206 30.1003V8.86074H99.1744L102.564 12.2575L101.402 7.12591H80.0878Z" fill="#FFFBF7"/>
                  <path d="M64.607 30.9679V19.9996H68.4852L70.7449 22.2694L70.7289 16.0597L68.4852 18.2971H64.607V8.86094H72.2433L75.6327 12.2576L74.4629 7.1261H57.3153L60.0557 9.84995V30.1005L57.4515 32.7028H75.3843L76.8347 26.6876L72.4516 30.9679H64.607Z" fill="#FFFBF7"/>
                  <path d="M62.6137 4.42626C63.8557 4.42626 64.8653 3.43725 64.8653 2.21313C64.8653 0.989019 63.8557 0 62.6137 0C61.3717 0 60.3621 0.989019 60.3621 2.21313C60.3621 3.43725 61.3717 4.42626 62.6137 4.42626Z" fill="#FFFBF7"/>
                  <path d="M70.5862 4.42626C71.8282 4.42626 72.8298 3.43725 72.8298 2.21313C72.8298 0.989019 71.8202 0 70.5862 0C69.3522 0 68.3346 0.989019 68.3346 2.21313C68.3346 3.43725 69.3442 4.42626 70.5862 4.42626Z" fill="#FFFBF7"/>
                </g>
                <g id="amp">
                ...
                <g id="chandon">
                  ...
                  <path d="M146.613 20.6641C146.14 14.1301 147.863 9.48496 151.517 8.84453C151.837 8.78778 152.11 8.73914 152.679 8.73914C156.485 8.77968 159.482 12.2331 160.427 13.9112L160.379 7.12591H153.039C146.725 7.12591 141.597 12.4844 141.597 19.821C141.597 27.1657 146.14 33.116 152.855 33.116C157.911 33.116 160.219 30.4732 161.773 27.5305C161.773 27.5305 159.177 29.7842 155.499 30.1328C151.044 30.5543 147.126 27.7494 146.613 20.6641Z" fill="#FFFBF7"/>
                  <path d="M202.637 6.62363H201.491L193.142 30.0926L190.506 32.7029H197.685L195.057 30.0926L197.629 22.756L204.176 22.7884L206.996 30.0926L204.376 32.7029H214.416L211.788 30.0926L202.629 6.62363H202.637ZM198.054 21.5075L200.73 13.9116L203.687 21.5075H198.054Z" fill="#FFFBF7"/>
                </g>
              </svg>
              <div ref={menuIconRef} onClick={() => setIsMenuOpen(!isMenuOpen)} className="pointer-events-auto cursor-pointer p-4 z-50 group">
                <div className="dots-container flex flex-col gap-[5px] items-center origin-center">
                  <div className="menu-dot w-[5px] h-[5px] rounded-full bg-[#FFFBF7] transition-colors" />
                  <div className="menu-dot w-[5px] h-[5px] rounded-full bg-[#FFFBF7] transition-colors" />
                </div>
              </div>
         </div>
